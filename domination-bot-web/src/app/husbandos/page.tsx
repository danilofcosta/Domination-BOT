      ...
      <header className="max-w-7xl mx-auto mb-10 md:mb-16 space-y-6 md:space-y-8">
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-4">
            <Link href="/waifus" className="px-4 py-2 md:px-5 md:py-2.5 bg-zinc-900 border border-zinc-800 rounded-2xl hover:bg-zinc-800 transition-all text-zinc-400 hover:text-white font-medium text-xs md:text-sm">
              Waifus
            </Link>
          </div>
        </div>
        ...
        <div className="space-y-3 md:space-y-4">
          <h1 className="text-4xl md:text-6xl font-black bg-gradient-to-r from-blue-400 to-indigo-600 bg-clip-text text-transparent tracking-tighter italic uppercase">
            Husbandos
          </h1>
        ...
        <div className="flex flex-col md:flex-row gap-4 pt-2 md:pt-4">
          <form className="relative flex-1 group">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 md:w-5 md:h-5 text-zinc-600 group-focus-within:text-blue-500 transition-colors" />
            <input 
              type="text" 
              name="q"
              placeholder="Pesquisar..."
              defaultValue={q}
              className="w-full bg-zinc-900/50 border border-zinc-800 rounded-2xl py-3 md:py-4 pl-10 md:pl-12 pr-4 focus:outline-none focus:border-blue-500/50 focus:bg-zinc-900 transition-all placeholder:text-zinc-700 text-sm md:text-base font-medium"
            />
          </form>
              ...
              <Link key={h.id} href={`/husbandos/${h.slug}`} className="group bg-zinc-900 border border-zinc-800 rounded-3xl overflow-hidden hover:border-blue-500/50 transition-all duration-500 shadow-2xl hover:shadow-blue-500/20 hover:-translate-y-2">
                <div className="aspect-[3/4] bg-zinc-800 relative overflow-hidden">
                  ...
                  <div className="absolute bottom-0 left-0 p-4 w-full">
                    <h3 className="font-extrabold truncate text-sm text-white drop-shadow-md group-hover:text-blue-400 transition-colors uppercase tracking-tight">{h.name}</h3>
                    <p className="text-[10px] text-zinc-500 uppercase tracking-widest mt-0.5 line-clamp-1 font-bold">{h.origem}</p>
                  </div>
                </div>
              </Link>
            ))
