...
from DB.models import ColecaoUsuarioHusbando, ColecaoUsuarioWaifu, Usuario, PersonagemHusbando, PersonagemWaifu
from domination.uteis import create_bts_y_or_n, dynamic_command_filter, send_media_by_type,COMMAND_LIST
...
async def gift_personagem(client: Client, message: Message):
    ...
    if not message.reply_to_message or message.reply_to_message.from_user.is_bot or message.reply_to_message.from_user.id == message.from_user.id:
        return await message.reply(
            await obter_mensagem_chat(client, message.chat.id, "gift", "reply_required"), quote=True
        )

    # Verifica se o ID do personagem foi enviado
    if len(message.command) < 2:
        return await message.reply(
            await obter_mensagem_chat(client, message.chat.id, "gift", "id_required", genero=client.genero.value), quote=True
        )

    try:
        personagem_id = int(message.command[1])
    except ValueError:
        return await message.reply(await obter_mensagem_chat(client, message.chat.id, "erros", "error_invalid_id"), quote=True)
    ...
    async with await client.get_reusable_session() as session:
        ...
        if not personagem:
            return await message.reply(
                await obter_mensagem_chat(client, message.chat.id, "erros", "error_character_not_found_id", 
                               genero=client.genero.value, personagem_id=personagem_id), quote=True
            )

        # Checa se o doador possui o personagem
        stmt = select(base).where(base.telegram_id == message.from_user.id, base.id_global == personagem_id)
        result = await session.execute(stmt)
        colecao = result.scalars().first()
        if not colecao:
            return await message.reply(await obter_mensagem_chat(client, message.chat.id, "erros", "error_character_not_owned"), quote=True)

    # Mensagem de confirmação para o doador enviar
    caption = await obter_mensagem_chat(client, message.chat.id, "gift", "confirmation_message",
...
async def gift_callback(client: Client, query):
    ...
    async with await client.get_reusable_session() as session:
        personagem = await session.get(personagem_cls, personagem_id)
        if not personagem:
            return await query.answer(await obter_mensagem_chat(client, query.message.chat.id, "erros", "error_character_not_found"), show_alert=True)

        if action == "send":
            # Remove da coleção do doador
            stmt_remove = select(base).where(base.telegram_id == giver_id, base.id_global == personagem_id)
            ...
            if not user:
                user_info = {"id": receiver_id, "NAME": query.message.chat.get_member(receiver_id).user.first_name, "username": None}
            ...
            await query.edit_message_caption(
                await obter_mensagem_chat(client, query.message.chat.id, "gift", "success_sent",
                               character_name=personagem.nome_personagem,
                               giver_mention=query.from_user.mention,receiver_mention=query.message.reply_to_message.from_user.mention)
            )

        else:
            await query.edit_message_caption(await obter_mensagem_chat(client, query.message.chat.id, "gift", "cancelled"))
