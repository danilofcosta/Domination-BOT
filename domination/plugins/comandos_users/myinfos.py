...
async def msg(client, chat_id, key, **kwargs):
    return     MESSAGE.get_text('pt',  "myinfos", key, **kwargs)
...
async def myinfos(client: Client, message:Message):
    ...
    membros = (
        ...
        # Ranking global
    ranking_global = (await DATABASE.get_info_all_objects(
    ...
    posicao_global = next((i + 1 for i, (tid, _) in enumerate(ranking_global) if tid == user_id), "-")
        ...
        # Ranking do grupo
    ranking_grupo = (await  DATABASE.get_info_all_objects(
            select(base.telegram_id, func.count(base.id_local).label("quantidade"))
    ...
    keyboard = InlineKeyboardMarkup(
        [[InlineKeyboardButton(await msg(client, chat_id, "delete_button"), callback_data="apagarharem_")]]
    )
    if file_id:
