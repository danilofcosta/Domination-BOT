...
# ===============================
async def create_keyboard(client, chat_id, items, page=1, per_page=14, prefix="item", back_data=None, is_character_list=False):
    start = (page - 1) * per_page
    end = start + per_page
    
    if is_character_list:
        # Para lista de personagens, criar botões com "Ver Personagem"
        view_text = await obter_mensagem_chat(client, chat_id, "listanime", "view_character")
        buttons = []
        ...
        buttons = [[InlineKeyboardButton(i, switch_inline_query_current_chat=f'list_anime_{i}')] for i in items[start:end]]
    
    nav = []
    total_pages = (len(items) + per_page - 1) // per_page
    if page > 1:
        previous_text = await obter_mensagem_chat(client, chat_id, "listanime", "previous")
        nav.append(InlineKeyboardButton(previous_text, callback_data=f"{prefix}_page_{page-1}"))
    if page < total_pages:
        next_text = await obter_mensagem_chat(client, chat_id, "listanime", "next")
        nav.append(InlineKeyboardButton(next_text, callback_data=f"{prefix}_page_{page+1}"))
    if back_data:
...
async def list_anime(client: Client, message):
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton(letter, callback_data=f"anime_letter_{letter}") for letter in row]
        for row in buttons
    ])
    select_text = await obter_mensagem_chat(client, message.chat.id, "listanime", "select_letter")
...
async def anime_letter_callback(client: Client, query: CallbackQuery):
    ...
    if not animes:
        no_anime_text = await obter_mensagem_chat(client, query.message.chat.id, "listanime", "no_anime_found")
    ...
    keyboard = await create_keyboard(client, query.message.chat.id, [anime[:25] for anime in animes], page=1, per_page=14, prefix=f"anime_{letter}", back_data="back_alphabet")
...
async def anime_select_callback(client: Client, query: CallbackQuery):
    ...
    if not anime_name:
        anime_not_found_text = await obter_mensagem_chat(client, query.message.chat.id, "listanime", "anime_not_found")
    ...
    if not personagens:
        no_characters_text = await obter_mensagem_chat(client, query.message.chat.id, "listanime", "no_characters_found")
    ...
    keyboard = await create_keyboard(client, query.message.chat.id, [p.nome_personagem[:25] for p in personagens], page=1, per_page=14, prefix=f"personagem_{key}", back_data=f"anime_letter_{anime_name[0]}", is_character_list=True)
    characters_text = await obter_mensagem_chat(client, query.message.chat.id, "listanime", "characters_from_anime", anime_name=anime_name)
...
async def back_alphabet_callback(client: Client, query: CallbackQuery):
    ...
    select_text = await obter_mensagem_chat(client, query.message.chat.id, "listanime", "select_letter")
