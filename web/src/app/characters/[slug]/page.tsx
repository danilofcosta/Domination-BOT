...
import { Calendar, Gem, ArrowLeft, Share2, SparklesIcon, ZapIcon } from "lucide-react";
...
async function getCharacter(slug: string) {
  ...
  if (typeHint === "waifu" || typeHint === "husbando") {
    const character = typeHint === "waifu" 
      ? await prisma.characterWaifu.findFirst({
          where: { slug: { equals: mainSlug, mode: "insensitive" } },
          include: { WaifuEvent: { include: { Event: true } }, WaifuRarity: { include: { Rarity: true } } }
        })
  ...
  if (!character) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4">
        <h1 className="text-4xl font-black italic opacity-20 mb-4">404_NOT_FOUND</h1>
        <Link href="/">
           <Badge variant="outline" className="px-6 py-2 cursor-pointer hover:bg-primary/10 transition-colors">Voltar ao Grid</Badge>
        </Link>
      </div>
    );
  }
      ...
      <div className="relative min-h-[85vh] w-full overflow-hidden flex flex-col group">
        ...
        <div className="relative z-10 max-w-7xl w-full mx-auto flex-1 flex flex-col justify-end pt-32 pb-16 px-6 lg:px-8">
          ...
          >
            ...
            <span className="font-black uppercase tracking-widest text-[10px] hidden sm:block">Frequência Central</span>
          </Link>

          <div className="flex flex-col lg:flex-row gap-12 lg:gap-20 items-center lg:items-end">
            
            {/* 🖼️ MAIN PORTRAIT */}
            <div className="relative w-64 sm:w-80 lg:w-[400px] shrink-0 aspect-[2/3] rounded-[2.5rem] overflow-hidden shadow-[0_0_80px_rgba(var(--primary-rgb),0.2)] border border-primary/10 group/img">
               <CharacterMedia 
                  item={character} 
                  type={character.type} 
               ...
               <div className="absolute top-6 right-6 flex flex-col gap-2">
                  {rarities.map((r: any) => (
                    <div
                      key={r.rarityId}
                      className="px-4 py-2 bg-background/60 backdrop-blur-xl border border-primary/10 rounded-2xl flex items-center gap-3 shadow-2xl animate-in fade-in slide-in-from-right duration-500"
                    >
               ...
               {/* ID Tag */}
               <div className="absolute bottom-6 left-6 py-1 px-4 bg-black/40 backdrop-blur-md rounded-full border border-white/10">
                  <span className="text-[10px] font-mono text-white/60">NODE_ID_{character.id}</span>
               </div>
            </div>
        ...
        <div className="lg:col-span-2 space-y-20">
          <section className="space-y-8">
            <div className="flex items-center gap-4">
              ...
              <h3 className="text-3xl font-black uppercase italic tracking-tighter">Sobre a Entidade</h3>
            </div>
            <p className="text-muted-foreground leading-relaxed text-xl font-medium max-w-3xl">
              Em destaque no universo de <span className="text-foreground underline decoration-primary/30">{character.origem}</span>, 
              o personagem <span className="text-foreground underline decoration-primary/30">{character.name}</span> é uma peça fundamental no ecossistema Domination. 
              Sincronizado originalmente em {new Date(character.createdAt).toLocaleDateString("pt-BR", { month: "long", year: "numeric", day: "numeric" })}.
            </p>
          </section>
          ...
          {events.length > 0 && (
            <section className="space-y-10">
               <div className="flex items-center gap-4">
                  ...
                  <h3 className="text-3xl font-black uppercase italic tracking-tighter">Eventos Ativos</h3>
               </div>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 {events.map((e: any) => (
                   ...
                   >
                     ...
                     <div>
                       <p className="text-lg font-black uppercase italic tracking-tighter leading-none mb-1">{e.Event.name}</p>
            ...
            <div className="space-y-6">
              <div className="flex justify-between items-center bg-background/40 p-6 rounded-3xl border border-white/5">
                <div className="space-y-1">
                   <span className="text-muted-foreground text-[10px] font-black uppercase tracking-widest block opacity-40">Popularidade</span>
                   <span className="text-lg font-black uppercase italic">Nível Global</span>
                </div>
              ...
              <div className="flex justify-between items-center bg-background/40 p-6 rounded-3xl border border-white/5">
                <div className="space-y-1">
                   <span className="text-muted-foreground text-[10px] font-black uppercase tracking-widest block opacity-40">Sincronização</span>
                   <span className="text-lg font-black uppercase italic">Cópias Ativas</span>
                </div>
              ...
              <div className="pt-4">
                 <div className="flex items-center gap-2 text-primary/40 text-[10px] font-bold uppercase tracking-widest justify-center">
                    <ZapIcon className="size-3" /> Atualizado há 1m
                 </div>
              </div>
            </div>
          </div>
