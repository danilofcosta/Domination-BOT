...
export default async function WaifuPage({ searchParams }: WaifuPageProps) {
  ...
  if (eventCode) where.WaifuEvent = { some: { Event: { code: eventCode } } };
  if (rarityCode) where.WaifuRarity = { some: { Rarity: { code: rarityCode } } };
  ...
  const [waifus, total, events, rarities] = await Promise.all([
    ...
    prisma.characterWaifu.count({ where: where as any }),
      ...
      <header className="w-full shrink-0 rounded-xl border border-border/70 bg-card/60 p-3 shadow-xs backdrop-blur-md sm:p-4">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h1 className="text-xl font-semibold tracking-tight sm:text-2xl">Waifus</h1>
          </div>
          <Link
            href="/characters"
      ...
      <div className="rounded-xl border border-border/70 bg-card/60 p-4 shadow-xs backdrop-blur-md">
        <div className="mb-3 flex items-center justify-between">
          <h2 className="text-lg font-semibold tracking-tight">
            {total} {total === 1 ? "waifu encontrada" : "waifus encontradas"}
          </h2>
        </div>

        {waifus.length === 0 ? (
          <p className="text-muted-foreground py-8 text-center text-sm">
            Nenhuma waifu encontrada com esses filtros.
          </p>
        ) : (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {waifus.map((w) => (
                  ...
                  >
                    <td className="px-2 py-2.5 text-center text-xs text-muted-foreground">{w.id}</td>
                    <td className="max-w-[180px] truncate px-2 py-2.5 font-medium">{w.name}</td>
                    <td className="max-w-[140px] truncate px-2 py-2.5 text-muted-foreground">{w.origem}</td>
                    <td className="px-2 py-2.5">
                      <div className="flex flex-wrap gap-1">
                        {w.WaifuRarity.length > 0
                          ? w.WaifuRarity.map((wr) => (
                              <span
                                key={wr.Rarity.code}
                                className="inline-flex items-center gap-0.5 rounded-md border border-border/50 bg-border/20 px-1.5 py-0.5 text-xs"
                                title={wr.Rarity.name}
                              >
                                {wr.Rarity.emoji} {wr.Rarity.name}
                              </span>
                            ))
