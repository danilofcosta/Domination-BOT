...
import { AddCharacterCollection } from "../../../utils/character/add_collection.js";
import {
  type MyContext,
  ChatType,
  ProfileType,
...
export async function addCollectionCallback(ctx: MyContext) {
  ...
  if (action === "s" || action === "n") {
    debug(`addCollectionCallback - ação recebida`, {
      userId,
      action,
      isMulti,
      adminId,
    ...
    if (action === "s") {
      ...
      if (isMulti) {
        ...
        if (!cachedData) {
          warn(`addCollectionCallback - dados multi não encontrados no cache`, {
            userId,
          });
      ...
      info(`addCollectionCallback - personagens adicionados por admin`, {
        adminId,
        userId,
        addedCount,
        failedCount,
    ...
    const msgText =
      action === "s"
        ? ctx.t(
            isMulti
              ? "addCollection-success-multi"
              : "addCollection-success-single",
            {
              user: mentionUser(
  ...
  if (isMulti) {
    ...
    if (!harem) {
      warn(`addCollectionCallback - harém não encontrado no cache`, { userId });
      await ctx.answerCallbackQuery(ctx.t("addCollection-cache-not-found"));
      return;
    }
    ...
    const keyboard = new InlineKeyboard()
      .text(
        ...
        `addCollection_${userId}_s_multi_${charIdsStr}`,
      )
      .text(ctx.t("addCollection-btn-no"), `addCollection_${userId}_n_multi`)
      .row()
      .switchInlineCurrent(
        ctx.t("addCollection-btn-view-harem"),
        `harem_user_${userId}`,
      );
    ...
    try {
      ...
      error(`addCollectionCallback - erro ao editar markup`, e);
    }
