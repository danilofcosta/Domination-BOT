...
export async function addCharacter_edit_CallbackData(
  ...
  const caption  = ctx.t(
    "edit_character_edit_caption",{
      nome: character.nome,
      anime: character.anime,
      genero: character.events ? "Husbando" : "Waifu",
      rarities: character.rarities?.length ? character.rarities.join(", ") : "valor padrao",
      events: character.events?.length ? character.events.join(", ") : "sem evento ",
