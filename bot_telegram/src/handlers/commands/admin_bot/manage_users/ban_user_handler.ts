...
export async function banHandler(ctx: MyContext) {
  ...
  info("banHandler - banindo usuario", {
    adminId: ctx.from?.id,
    targetId: result.id,
  });
...
export async function unbanHandler(ctx: MyContext) {
  ...
  if (!result?.id) {
    return Sendmedia({
      ctx,
      caption: ctx.t("banuser-usage-unban", { prefix: botPrefix }),
    });
  }
  ...
  if (targetRole && roleWeights[targetRole] >= roleWeights[ProfileType.ADMIN]) {
    ...
    return Sendmedia({
      ctx,
      caption: ctx.t("banuser-cannot-unban-admin"),
    });
  }

  info("unbanHandler - desbanindo usuario", {
    adminId: ctx.from?.id,
    targetId: result.id,
  });
  ...
  if (!user) {
    return Sendmedia({
      ctx,
      caption: ctx.t("banuser-not-found"),
    });
  }
  ...
  const targetName =
    targetData?.first_name || targetData?.username || result.id.toString();

  return await Sendmedia({
    ctx,
    caption: ctx.t("banuser-success-unban", {
      name: targetName,
      id: result.id,
    }),
    reply_markup: CreateOneBtn({
...
export async function listBannedHandler(ctx: MyContext) {
  ...
  try {
    const banned = await prisma.telegramUser.findMany({
