...
export async function HaremmodeHandler(ctx: MyContext) {
  const character = await LastRandomCharacter(ctx.botType);
  ...
  const keyboard = new InlineKeyboard()
    .text(
      `${currentMode === "default" || !currentMode ? "✅ " : ""}${ctx.t("haremmode-default")}`,
      "haremmode_default",
    )

    .text(
      `${currentMode === "latest" || !currentMode ? "✅ " : ""}${ctx.t("haremmode-recent")}`,
      "haremmode_latest",
    )
    .row()
    .text(
      `${currentMode === "rarity" ? "✅ " : ""}${ctx.t("haremmode-rarity")}`,
      "haremmode_rarity",
    )

    .text(
      `${currentMode === "event" ? "✅ " : ""}${ctx.t("haremmode-event")}`,
      "haremmode_event",
    )
    .row()
