...
async function showAnimeList(
  ...
  if (animes.length === 0) {
    await ctx.answerCallbackQuery(ctx.t("animelist-no-anime", { letter }));
  ...
  let text =
    ctx.t("animelist-header", { letter, total: String(animes.length) }) +
    "\n\n";
  text +=
    ctx.t("animelist-page", {
  ...
  keyboard.push([
    { text: ctx.t("animelist-btn-back"), callback_data: "al_back_" + cacheKey },
  ]);
