...
export default function EventsPage() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setEditOpen(false)
      setEditItem(null)
      toast("Evento atualizado", { description: `${editEmoji} ${editName} foi salvo com sucesso.` })
    } catch (e) {
    ...
    <div className="bg-background">
      <div className="mx-auto w-full 2xl:max-w-[1600px] px-6 py-8 pt-16 lg:pt-8">
        <header className="mb-8">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Eventos especiais e campanhas</p>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {(events ?? []).map((event) => (
              <Card key={event.id} className="group relative">
                <Button variant="ghost" size="icon" className="absolute right-2 top-2 opacity-0 transition-opacity group-hover:opacity-100" onClick={() => openEdit(event)}>
            ...
            <DialogFooter className="border-t bg-card px-6 py-4 mt-0">
              ...
              <Button onClick={async () => {
                try {
                  ...
                  toast("Evento criado", { description: `${createEmoji} ${createName} foi adicionado com sucesso.` })
                } catch (e) {
